...
vim.keymap.set('n', 'R', function()
  vim.call("<SID>idris(':reload')")
end, { buffer = true, silent = true })
vim.keymap.set('n', 'T', function()
  vim.call("<SID>idris_cword(':type')")
end, { buffer = true, silent = true })
vim.keymap.set('n', 'd', function()
  vim.call("<SID>idris_cword(':doc')")
end, { buffer = true, silent = true })
vim.keymap.set('n', 'm', function()
  vim.call("<SID>idris_cword(':missing')")
end, { buffer = true, silent = true })
vim.keymap.set('n', 'o', function()
  vim.cmd("<C-u>vsp<CR>:IdrisRepl -p base -p contrib -p effects <C-r>=expand('%:p')<CR>")
end, { buffer = true, silent = true })
vim.keymap.set('n', 'p', function()
  vim.call("<SID>idris_clnum_word(':proofsearch', '<C-r>=expand('<cword>')[1:]<CR>')")
end, { buffer = true, silent = true })
