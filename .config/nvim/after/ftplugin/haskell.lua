...
vim.keymap.set('n', 'R', function()
  vim.call('<SID>stack_test_tasty()')
end, { buffer = true, silent = true })
vim.keymap.set('n', 'R', function()
  vim.call('<SID>stack_test_doctest()')
end, { buffer = true, silent = true })
-- NOTE: v  This is useful for that is building happy codes and show its warnings/errors on vim-ghcid-quickfix
vim.keymap.set('n', 'b', function()
  vim.call(
    "vimrc#open_terminal_as('stack_build', 'vertical', 'stack build --fast', {'noclose': v:true, 'path': g:vimrc.path_at_started})"
  )
end, { buffer = true, silent = true })
vim.keymap.set('n', 'w', function()
  vim.call('<SID>ghcid_quickfix_start_on_path_started()')
end, { buffer = true, silent = true })
vim.keymap.set('n', 'W', function()
  vim.cmd('<C-u>GhcidQuickfixStop')
end, { buffer = true, silent = true })
vim.keymap.set('n', 'T', function()
  vim.call('<SID>stack_integrate_test_or_unit_or_both()')
end, { buffer = true, silent = true })
vim.keymap.set('n', 't', function()
  vim.cmd('<C-u>GhcidQuickfixStart "--command=stack ghci :tasty"')
end, { buffer = true, silent = true })
