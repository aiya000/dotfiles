local function find_tsdk(root_dir)
  ...
  if npm_root and npm_root ~= '' then
    local npm_tsdk = npm_root .. '/typescript/lib'
    if vim.uv.fs_stat(npm_tsdk) then
      return npm_tsdk
    end
  end

  -- bun
  local bun_tsdk = vim.fn.expand('~/.bun/install/global/node_modules/typescript/lib')
  if vim.uv.fs_stat(bun_tsdk) then
    return bun_tsdk
  end
