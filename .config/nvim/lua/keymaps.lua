...
map('n', '{', '{zv')
map('n', '}', '}zv')
map('n', ',,', 'ggVG"+y<C-o>') -- Copy the buffer content to clipboard
map('n', ',<', 'ggVG"_s') -- Refresh the buffer content
map('n', '<C-x><C-n>', '<C-n>')
map('n', '<C-x><C-p>', '<C-p>')
map('n', '/', [[/\m]])
map('n', '?', [[?\m]])
...
map('n', 'M', '<Cmd>Telescope frecency<CR>', { silent = true })
