...
vim.list_extend(
  haskell_snippets,
  sm({ 'deriving', 'der', 'deri' }, { t('deriving ('), i(0, 'here'), t(')') }, { key = 'deriving' })
)
...
table.insert(
  haskell_snippets,
  s('instance', { t('instance '), i(1, 'Class'), t(' '), i(2, 'Name'), t(' where'), i(0) })
)
...
-- Import snippets
vim.list_extend(haskell_snippets, sm({ 'import', 'imp' }, { t('import '), i(1) }, { key = 'import' }))

vim.list_extend(
  haskell_snippets,
  sm({ 'import_qualified', 'imq' }, { t('import qualified '), i(1), t(' as '), i(2) }, { key = 'import_qualified' })
)
...
vim.list_extend(haskell_snippets, sm({ 'to', 'constraints_for' }, t('=>'), { key = 'to' }))
...
-- Comment snippets
vim.list_extend(haskell_snippets, sm({ 'comment_block', 'com' }, { t('{-'), i(1), t('-}') }, { key = 'comment_block' }))

vim.list_extend(
  haskell_snippets,
  sm({ 'document_comment', 'haddock_comment', 'doc' }, t('-- |'), { key = 'document_comment' })
)

vim.list_extend(
  haskell_snippets,
  sm({ 'document_comment_back', 'doc_back', 'back_doc', 'bak', 'doca' }, t('-- ^'), { key = 'document_comment_back' })
)
...
table.insert(
  haskell_snippets,
  s('liquid_refinement_type_type', { t('{-@ type '), i(1, 'typeName'), t(' = '), i(2), t(' @-}') })
)
...
vim.list_extend(
  haskell_snippets,
  sm({ 'quasi_quote', 'q' }, { t('['), i(1, 'r'), t('|'), i(0), t('|]') }, { key = 'quasi_quote' })
)

vim.list_extend(
  haskell_snippets,
  sm({ 'quasi_quote_i', 'qi', 'i' }, { t('[i|'), i(0), t('|]') }, { key = 'quasi_quote_i' })
)

vim.list_extend(
  haskell_snippets,
  sm({ 'quasi_quote_r', 'qr', 'r' }, { t('[r|'), i(0), t('|]') }, { key = 'quasi_quote_r' })
)

vim.list_extend(
  haskell_snippets,
  sm({ 'quasi_quote_s', 'qs', 's' }, { t('[s|'), i(0), t('|]') }, { key = 'quasi_quote_s' })
)

vim.list_extend(
  haskell_snippets,
  sm({ 'quasi_quote_here', 'qhere', 'here' }, { t('[here|'), i(0), t('|]') }, { key = 'quasi_quote_here' })
)
...
vim.list_extend(
  haskell_snippets,
  sm({ 'println_for', 'println', 'pr', 'print' }, { t('print $ '), i(0) }, { key = 'println_for' })
)

vim.list_extend(haskell_snippets, sm({ 'putStrLn', 'puts', 'ps' }, t('putStrLn '), { key = 'putStrLn' }))

-- Lens snippets
vim.list_extend(haskell_snippets, sm({ 'lens_get', 'get' }, t('^.'), { key = 'lens_get' }))

vim.list_extend(haskell_snippets, sm({ 'lens_set', 'set' }, t('.~'), { key = 'lens_set' }))

vim.list_extend(haskell_snippets, sm({ 'prism_get', 'look', 'lk' }, t('^?'), { key = 'prism_get' }))
