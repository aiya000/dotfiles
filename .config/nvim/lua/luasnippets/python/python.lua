...
vim.list_extend(
  python_snippets,
  sm({'conditional_operator', 'cond'}, fmt('{} if {} else {}', { i(1, 'value_if_true'), i(2, 'cond'), i(3, 'value_if_false') }))
)
...
vim.list_extend(
  python_snippets,
  sm({'__init__', 'init'}, fmt([[def __init__(self{}) -> None:
    {}]], { i(1, '#:, x'), i(0) }))
)

vim.list_extend(
  python_snippets,
  sm({'__post_init__', 'post_init'}, fmt([[def __post_init__(self{}) -> None:
    {}]], { i(1, '#:, x'), i(0) }))
)

vim.list_extend(
  python_snippets,
  sm({'__str__', '__str'}, fmt([[def __str__(self) -> str:
    {}]], { i(0) }))
)
