...
describe('utils.functions', function()
  describe('set_vim_dict_field()', function()
    it('should set a sub field', function()
      ...
      Functions.set_vim_dict_field(dict, 'field', 'sub_field', 10)
  ...
  describe('to_pretty_string()', function()
    it('should convert primitives to string', function()
      assert.are.equal('42', Functions.to_pretty_string(42))
      assert.are.equal('hello', Functions.to_pretty_string('hello'))
      assert.are.equal('true', Functions.to_pretty_string(true))
      assert.are.equal('false', Functions.to_pretty_string(false))
      assert.are.equal('nil', Functions.to_pretty_string(nil))
    end)

    it('should convert simple table to string', function()
      local result = Functions.to_pretty_string({ name = 'John', age = 30 })
    ...
    it('should handle nested tables', function()
      ...
      local result = Functions.to_pretty_string(nested)
    ...
    it('should handle empty table', function()
      local result = Functions.to_pretty_string({})
      assert.are.equal('{\n}', result)
    end)
  end)
  ...
  describe('make_table_to_string()', function()
    it('should work directly', function()
      local result = Functions.make_table_to_string({ key = 'value' })
      assert.are.equal("{\n 'key' = 'value',\n}", result)
    end)

    it('should handle numeric keys', function()
      local result = Functions.make_table_to_string({ [1] = 'first', [2] = 'second' })
