...
function GetVimIndentOverwrite()
  ...
  elseif
    vim.fn.getline(lnum):match('^%s*aug%[roup]') and not vim.fn.getline(lnum):match('^%s*aug%[roup]%s*!?%s+END')
  then
    ind = ind + vim.bo.shiftwidth
  end
  ...
  if (i and i > 0) and not line:match('^%s*au%[tocmd]') then
    if
      not vim.fn.has('syntax_items')
      or not vim.fn.synIDattr(vim.fn.synID(lnum, i + 2, 1), 'name'):match('(Comment|String)$')
    then
      ind = ind - vim.bo.shiftwidth
    end
  end
