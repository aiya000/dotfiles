...
3. **No fabricated facts.** If a behavior, version, benchmark, or link was not verified in this session, either verify it (read the code, run it, fetch the doc) or mark it in the article the way the user does: 「（実働を確認しておりません）」「（未検証）」 or a `TODO:` line. Never invent StackBlitz URLs, PR numbers, or output.
    ...
    - A claim taken from the user's own commit message (a measurement, a bug, a "this turned out to be too few") is still unverified until reproduced. Reproduce it when the environment allows; otherwise cite the commit hash and write 「この記事のための再現はしていません（未検証）」. When the reproduction gives a different number, write both and say which is which (VRCWW, 2026-09-23: the merge-engine commit said 40 tests, `vitest` on the current file passed 35; the article carries 「追加時のコミットでは40本、手元では35本」).
    - Commit links carry the hash exactly as `git log` printed it. Never pad a short hash out to 40 characters by hand; a full hash that was not copied from `git log --format=%H` is a fabrication, and GitHub resolves `/commit/<short hash>` as it is (VRCWW, 2026-09-23: `df79d82` was nearly linked as an invented 40-character hash).
...
4. Collect inputs from the user's request and the workspace:
    ...
    - screenshots that already live in the tool's public repository (a README's `docs/screenshots/`) are embedded by `https://raw.githubusercontent.com/<owner>/<repo>/<default branch>/<path>` when the user agrees, so nothing has to be uploaded to the platform. `curl -sS -o /dev/null -w '%{http_code}'` one of them first: the path exists on the default branch, not necessarily on the branch being read
    - when the choices are put to the user with `AskUserQuestion`, read the free-text 「Other」 answer as carefully as the ticked options: it may add material or a whole section the list did not offer (VRCWW, 2026-09-23: the user added 「フィッシングサイト認定された際の対応行程」 there, and it became the section the review singled out)
...
7. **Dig the story out of git before drafting a tool article** (patterns A and C). Run `git log --format='%h %ad%n%B' --date=short -- <file>` on the tool: the commit messages hold the motivation, the measurements, the wrong turns, and the reverts that the README does not. Then:
    ...
    - When the repository root looks empty or sits on a throwaway branch, run `git worktree list` before concluding anything: the user keeps some repositories as a parent directory of worktrees, with the real work in a sibling such as `develop/` (VRCWW, 2026-09-23: the root was on a `dummy` branch and held no README)
    - Dates: `git log` prints local time (JST) while `gh release view` / the GitHub API print UTC, so a release published late evening UTC is the next day in JST. Convert before writing a 日付 into the article (VRCWW, 2026-09-23: v3.0.0 was written as 9月18日 from `publishedAt` and is 9月19日 in JST)
    - Incidents and operations (an outage, a false-positive block, a review that was refused three times) leave no trace in `git log` beyond the fix commit. Their timeline, and the order of events that decides what may honestly be claimed, live in the user's memory files (`~/.ai-memory/MEMORY-INDEX.md`, then the file it points at) and handoff notes (`~/tmp/claude-handoff/`). Read them for pattern C, and keep the order they record (VRCWW, 2026-09-23: the Safe Browsing warning cleared before the fix reached production, so the article says the fix did not clear it)
...
- Commit links: `rg -o '/commit/[0-9a-f]{40}' <file>` lists every full hash; each must appear verbatim in `git log --format=%H` output from this session, or be shortened to what `git log` printed (requirement 3)
...
- Zenn's AI review of six pattern C articles (2026-09-06: `expects`, bash-toys, `ps-mem`; 2026-09-12: chotto.lua, CopyMenu; 2026-09-23: VRCWW; full text in `references/review-log.md`) listed no improvements for any of them, and praised the same things each time. Keep them in every pattern C article:
    ...
    - an incident from running the thing in production is a `###` of the same rank as a design decision: what was observed, what the dashboards said, what was changed, and what may not be claimed (VRCWW: the Safe Browsing false positive, with 「修正したから消えたとは言えません」 kept in). The review named it as what made the article 「リアル」
    - a migration article (desktop → web, one runtime → another) opens its technical part with a before/after table, one row per responsibility (persistence, logic, API, auth, updates, links), and the `###` that follow each answer one row of it (VRCWW: the eight-row Tauri/Web table, then Cookie relay, Service Worker cache, Dexie tombstones). The review called the trade-offs 「非常に明確」
