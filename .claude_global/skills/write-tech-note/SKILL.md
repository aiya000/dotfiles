...
11. Report: the platform and which marker in the current directory decided it (or that the user chose it), the file path, the pattern and register used, every unverified claim you marked, every `TODO:` left in the file, how the demo output was captured, and what the user should look at when they 推敲 (the user reviews and reworks every article before it goes out, so point them at the weak spots). Leave the demo scripts and `out-*.txt` files in the scratchpad and say so, so the user can re-shoot.
