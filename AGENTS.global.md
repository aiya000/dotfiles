...
メモリーファイルの目的は、将来のセッションが素早く文脈を復元できるようにすることです。
日記形式ではなく、次のセッションで読み込むための記憶として記述します。

ファイルが存在しない場合はヘッダー `# Memory - YYYY-MM-DD` で始める新規ファイルを作成し、
既に存在する場合は現在時刻のタイムスタンプで新しいエントリを追記します。

エントリの形式: `## [HH:MM] - <トピックの短いタイトル>`

残すべき内容：
- 決定・実装した内容（経緯ではなく結果）
- セッション中に判明したユーザーの好みや制約
- コードや git 履歴から分からないプロジェクト固有の事実
- 発生したエラーとその解決方法

除外する内容：
- ツール呼び出しのログやコマンド出力
- 長いコードブロック（本当に必要な短いスニペット以外）
- 往復の確認会話
