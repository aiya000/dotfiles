function! vimrc#dein#hook_source#gina() abort " {{{
    call gina#custom#mapping#nmap('status' , 'A'     , ':<C-u>call vimrc#dein#hook_source#gina_git_xxx_patch_this("add")<CR>'        , {'noremap': 1 , 'silent': 1})
    ...
    call gina#custom#mapping#nmap('status' , 'sa'    , ':<C-u>call vimrc#dein#hook_source#gina_git_xxx_patch_this("stash push")<CR>' , {'noremap': 1 , 'silent': 1})
...
" Expects 'add' or 'stash' as a:subcmd (xxx)
function! vimrc#dein#hook_source#gina_git_xxx_patch_this(subcmd) abort " {{{
  "TODO: This may fail if the file name contains spaces
  let file = split(getline('.'))[2]
  CdGitRoot
  execute 'terminal' 'git' a:subcmd  '--patch' file
endfunction " }}}
